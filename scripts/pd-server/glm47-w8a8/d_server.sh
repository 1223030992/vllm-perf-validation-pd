...
usage() {
  cat <<'USAGE'
Usage:
  d_server.sh --model-path PATH --port PORT --vllm-host-ip IP \
    --gpu-range RANGE --tp TP [--network-ifname IFNAME] \
    [--nccl-ib-hca HCA] [--quantization NAME] [--dtype DTYPE] \
...
MODEL_PATH=""; PORT=""; VLLM_HOST_IP=""; GPU_RANGE=""; TP=""
