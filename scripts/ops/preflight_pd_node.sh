...
check_node() {
  ...
  run_ssh "$node" model_path "test -d $(quote_sh "$HOST_MODEL_PATH") && echo 'model path exists' || echo 'WARN: host model path not found; continuing because container model mount may still be valid'"
...
run_ssh "$PROXY_NODE" mooncake_proxy_script "test -f $(quote_sh "${SKILL_HOST_ROOT%/}/${MOONCAKE_PROXY_SCRIPT#./}") && echo 'mooncake proxy script exists' || (echo 'MOONCAKE_PROXY_SCRIPT_NOT_FOUND: ${SKILL_HOST_ROOT%/}/${MOONCAKE_PROXY_SCRIPT#./}' >&2; exit 1)"
