...
usage() {
  cat <<'USAGE'
Usage:
  run_pd_task.sh --config CONFIG --user USER --abbr ABBR \
...
CONFIG=""; CLEANUP_STATE=""; ASSUME_YES=0; DRY_RUN=0; KEEP_CONTAINERS_ON_FAILURE=0
...
if [[ -n "$CLEANUP_STATE" ]]; then
  [[ -z "$CONFIG" && -z "$IMAGE_ARG" ]] || { echo "cleanup_state_conflicts_with_run_args=1" >&2; exit 2; }
...
[[ -n "$CONFIG" ]] || { echo "missing_arg=--config" >&2; exit 2; }
resolve_runtime_config
CONFIG_PATH="$CONFIG"; [[ "$CONFIG_PATH" == /* ]] || CONFIG_PATH="$SKILL_ROOT/$CONFIG_PATH"
...
TP="${PD_SERVICE_DEFAULTS_TP:-8}"; GPU_RANGE="${PD_SERVICE_DEFAULTS_GPU_RANGE:-0,1,2,3,4,5,6,7}"
QUANTIZATION="${PD_SERVICE_DEFAULTS_QUANTIZATION:-slimquant_marlin}"; DTYPE="${PD_SERVICE_DEFAULTS_DTYPE:-bfloat16}"
...
if [[ "$DRY_RUN" == "0" ]]; then
  run_stage initialize_state "$TMP_DIR/initialize_state.out" update_state_host --set "status=INITIALIZED" --set "config.path=$CONFIG_PATH" --set "image=$IMAGE" --set "ops.version=$SCRIPT_VERSION" \
